...
(function() {
  $(function() {
    var itemize, itemizeAndRemove, reverse, selector;
    reverse = function(el) {
      return $($(el).get().reverse());
    };
    itemize = function(el) {
      var cost, html, i, len, name, tag, tags;
      ...
      if ((cost != null ? cost.length : void 0) > 0) {
        html += "<span class='cost'>" + cost + "</span>";
      }
      return html += "</div>";
    };
    itemizeAndRemove = function(el) {
      var html, index;
      html = itemize(el);
      $(".items-list").prepend("<div class='has_many_item'></div>");
      index = $("body").hasClass("admin_namespace") ? "last" : "first";
      $(".has_many_item:first:" + index).append($(el).hide());
      $(".has_many_item:first:" + index).append(html);
      return $(".items-list .item-summary").off('click').on('click', function() {
        return $(this).parent(".has_many_item").find('.has_many_fields').show((function(_this) {
          return function() {
            return $(_this).hide();
          };
        })(this));
    ...
    if (selector.length > 0 && !selector.hasClass("index")) {
      $("#footer").hide();
      $(".actions").insertAfter(".place-details");
      $('html, body').animate({
        scrollTop: $(".actions").offset().top
      }, 1000);
      $('.has_many_add').after("<div class='items-list'></div>");
      $(".has_many_fields:visible").each(function() {
        return itemizeAndRemove(this);
      });
      $(".has_many_item").filter(function(index) {
        return $(this).find(".field_with_errors").length > 0;
      }).each(function() {
        $(this).find(".has_many_fields").show();
        return $(this).find(".item-summary").hide();
      });
      $('body').on('has_many_add:before', function(e, parent) {
        return parent.data('has_many_index', $(".items-list .has_many_item").length - 1);
      });
      return $('body').on('has_many_add:after', function(e, fieldset, parent) {
        ...
        if ($(".has_many_fields:visible").length > 1 && $(".item-name:visible:first").val().length > 0) {
          return itemizeAndRemove('.has_many_fields:visible:first');
        } else {
          return $(".items-list").prepend("<div class='has_many_item' style='display:none'></div>");
        }
      });
    }
  });
